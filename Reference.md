...
严格来说，高分辨率定时器并没有对应的物理硬件，它还是基于硬件通用定时器的，只不过对中断事件的管理功能。因为硬件通用计时器只有"比较然后生成中断"这一个功能，而高分辨率定时器额外为中断增加了一些管理功能，比如延时触发、丢弃重复触发等等。
...
因为i2s的发送和接收的时钟是一致的，所以如果发送和接收配置时钟不同时，需要占用多个i2s。并且如果设置为24bit的数据的话，需要重新设置下时钟，否则ws会不准确，同时初始化失败。
